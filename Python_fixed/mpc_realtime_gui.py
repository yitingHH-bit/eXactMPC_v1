...
class ExcavatorRealtimeGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # Minimal clean theme for ttk widgets
        style = ttk.Style()
        try:
            style.theme_use("clam")
        except Exception:
            pass
        style.configure("TLabel", font=("Segoe UI", 10), padding=2)
        ...
        # ----- 3D canvas -----
        self.fig = Figure(figsize=(7.2, 5.4), facecolor="#f9fafb")
        self.ax = self.fig.add_subplot(111, projection="3d")
        self.canvas = FigureCanvasTkAgg(self.fig, master=self.root)
        self.canvas_widget = self.canvas.get_tk_widget()
        self.canvas_widget.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        # Optional frame saving (set MPC_SAVE_FRAMES=1 to enable)
        self.save_frames = bool(int(os.getenv("MPC_SAVE_FRAMES", "0")))
        self.frames_dir = os.path.join(os.path.dirname(__file__), "Plots")
        if self.save_frames:
        ...
        ttk.Checkbutton(row1, text="Show Gripper", variable=self.show_gripper_var).pack(side=tk.LEFT, padx=8)
        ...
        self.target_y_var = tk.DoubleVar(value=0.00)
        ...
        ttk.Label(row3, text="Target Y [m]:").pack(side=tk.LEFT, padx=(12, 0))
        ttk.Entry(row3, textvariable=self.target_y_var, width=10).pack(side=tk.LEFT, padx=3)
        ...
        self.target_xyz = np.array([0.70, 0.0, float(C.yGround)], dtype=float).reshape(3,)
        self.target_phi = 0.0
        ...
        ttk.Label(info, text="Desired (x,y,z,phi):").grid(row=1, column=0, sticky="w")
        ttk.Label(info, textvariable=self.des_x_var, width=10).grid(row=1, column=1, sticky="w")
        ttk.Label(info, textvariable=self.des_y_var, width=10).grid(row=1, column=2, sticky="w")
        ttk.Label(info, textvariable=self.des_z_var, width=10).grid(row=1, column=3, sticky="w")
        ttk.Label(info, textvariable=self.des_phi_var, width=10).grid(row=1, column=4, sticky="w")

        ttk.Label(info, text="Pred end (x,y,z,phi):").grid(row=2, column=0, sticky="w")
        ttk.Label(info, textvariable=self.pred_x_var, width=10).grid(row=2, column=1, sticky="w")
        ttk.Label(info, textvariable=self.pred_y_var, width=10).grid(row=2, column=2, sticky="w")
        ttk.Label(info, textvariable=self.pred_z_var, width=10).grid(row=2, column=3, sticky="w")
        ttk.Label(info, textvariable=self.pred_phi_var, width=10).grid(row=2, column=4, sticky="w")

        ttk.Label(info, text="Current (x,y,z,phi):").grid(row=3, column=0, sticky="w")
        ttk.Label(info, textvariable=self.cur_x_var, width=10).grid(row=3, column=1, sticky="w")
        ttk.Label(info, textvariable=self.cur_y_var, width=10).grid(row=3, column=2, sticky="w")
        ttk.Label(info, textvariable=self.cur_z_var, width=10).grid(row=3, column=3, sticky="w")
        ttk.Label(info, textvariable=self.cur_phi_var, width=10).grid(row=3, column=4, sticky="w")

        ttk.Label(info, text="Error (x,y,z,phi):").grid(row=4, column=0, sticky="w")
        ttk.Label(info, textvariable=self.err_x_var, width=10).grid(row=4, column=1, sticky="w")
        ttk.Label(info, textvariable=self.err_y_var, width=10).grid(row=4, column=2, sticky="w")
        ttk.Label(info, textvariable=self.err_z_var, width=10).grid(row=4, column=3, sticky="w")
        ttk.Label(info, textvariable=self.err_phi_var, width=10).grid(row=4, column=4, sticky="w")
    ...
    def set_target_pose(self, silent=False):
        ...
        if r > r_max + 1e-6:
            ...
            y *= scale
            if not silent:
                self.status_var.set(f"Target clipped to reach limit: x={x:.3f}, y={y:.3f}")
            # reflect clipped values in entries
            self.target_x_var.set(x)
            self.target_y_var.set(y)

        self.target_xyz = np.array([x, y, z], dtype=float).reshape(3,)
        self.target_phi = float(phi)

        self.des_x_var.set(f"{x:.3f}")
        self.des_y_var.set(f"{y:.3f}")
        self.des_z_var.set(f"{z:.3f}")
        self.des_phi_var.set(f"{phi_deg:.1f}")

        if not silent:
            self.status_var.set(f"Target set: x={x:.3f}, y={y:.3f}, z={z:.3f}, phi={phi_deg:.1f} deg")
        return True
    ...
    def link_positions_3d(self, yaw, q):
        ...
        In X plane, +Y rotation corresponds to rotating the vector by -angle.
        So we use:
            theta1 = OFF1 - q1
            theta2 = OFF2 - (q1+q2)
    ...
    def draw_frame(self, x_vec, t):
        ...
        try:
            pose_planar = _dm_to_np(mod.forwardKinematics(csd.DM(q).reshape((3, 1))))
        except Exception:
            pose_planar = np.array([], dtype=float)

        if pose_planar.size == 3 and _is_finite(pose_planar):
            x_c, z_c, phi_c = pose_planar
            x_w = x_c * math.cos(yaw)
            y_w = x_c * math.sin(yaw)
            z_w = z_c

            self.cur_x_var.set(f"{x_w:.3f}")
            self.cur_y_var.set(f"{y_w:.3f}")
            self.cur_z_var.set(f"{z_w:.3f}")
            self.cur_phi_var.set(f"{math.degrees(phi_c):.1f}")

            des = getattr(self, "target_xyz", np.zeros(3, dtype=float))
            self.err_x_var.set(f"{(x_w - des[0]):+.3f}")
            self.err_y_var.set(f"{(y_w - des[1]):+.3f}")
            self.err_z_var.set(f"{(z_w - des[2]):+.3f}")
            self.err_phi_var.set(f"{math.degrees(phi_c - getattr(self, 'target_phi', 0.0)):.1f}")
        else:
            self.cur_x_var.set("--")
            self.cur_y_var.set("--")
            self.cur_z_var.set("--")
        ...
        arm_color = "#2d9cdb"
        target_color = "#f2c94c"
        gripper_color = "#f2994a"
        self.ax.set_facecolor("#f9fafb")
        self.ax.grid(color="#dfe3eb", alpha=0.6, linewidth=0.8)
        self.ax.plot(pts[:, 0], pts[:, 1], pts[:, 2], "-o", color=arm_color, linewidth=2.2, markersize=6, label="excavator arm")

        if len(self.tip_traj) > 1:
            traj = np.vstack(self.tip_traj)
            self.ax.plot(traj[:, 0], traj[:, 1], traj[:, 2], "--", linewidth=1.4, color="#95a5a6", label="tip trajectory")

        # target star (desired) in world
        try:
            xd, yd, zd = map(float, self.target_xyz.reshape(3,))
            self.ax.scatter([xd], [yd], [zd], marker="*", s=120, color=target_color, edgecolors="k", linewidths=0.5, label="target")
        except Exception:
            pass
        ...
        if self.pred_pose_last is not None and self.pred_yaw_last is not None:
            try:
                xw, yw, zw = map(float, self.pred_pose_last.reshape(3,))
                self.ax.scatter([xw], [yw], [zw], marker="x", s=70, color="#e74c3c", label="mpc_plan_end")
            except Exception:
                pass
        ...
        if self.show_gripper_var.get():
            try:
                ...
                self.ax.plot(PwL[:, 0], PwL[:, 1], PwL[:, 2], linewidth=3, color=gripper_color, label="gripper")
                self.ax.plot(PwR[:, 0], PwR[:, 1], PwR[:, 2], linewidth=3, color=gripper_color, label="_nolegend_")
            except Exception:
                pass

        # ground line (z = yGround)
        L = max(self.reach_max + 0.3, 1.2)
        self.ax.plot([-L, L], [0, 0], [float(C.yGround), float(C.yGround)], linestyle="--", color="#7f8c8d", linewidth=1.2, label="ground")
        ...
        self.ax.set_title(f"Realtime MPC  t={t:.2f}s  k={self.k}", pad=12, fontsize=12, fontweight="bold")
        self.ax.view_init(elev=24, azim=-58)
        self.ax.legend(loc="upper left", fontsize=8, frameon=True, framealpha=0.9)
    ...
    def step_mpc(self):
        ...
        # validate target
        try:
            self.target_xyz = np.asarray(self.target_xyz, dtype=float).reshape(3,)
            if not _is_finite(self.target_xyz):
                raise ValueError("target_xyz not finite")
            self.target_phi = float(self.target_phi)
        except Exception:
        ...
        # solve NLP
        try:
            sol = self.opti.solveNLP(x_now, self.target_xyz, self.target_phi)
        except Exception as e:
        ...
        try:
            ...
            if poseN.size == 3 and _is_finite(poseN):
                xwN = poseN[0] * math.cos(yawN)
                ywN = poseN[0] * math.sin(yawN)
                zwN = poseN[1]
                self.pred_pose_last = np.array([xwN, ywN, zwN], dtype=float)
                self.pred_yaw_last = yawN
                self.pred_x_var.set(f"{xwN:.3f}")
                self.pred_y_var.set(f"{ywN:.3f}")
                self.pred_z_var.set(f"{zwN:.3f}")
    ...
    def update_frame(self):
        ...
        if self.save_frames:
            try:
                fname = os.path.join(self.frames_dir, f"frame_{self.k:04d}.png")
