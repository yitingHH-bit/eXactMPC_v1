...
class NLP:
    ...
    def solveNLP(self, x_current, pose_desired):
        ...
        # - Users often pass CasADi DM/MX, which can become an object array of size 1.
        # - We always flatten first, validate sizes, then reshape.
        x_current = np.asarray(x_current, dtype=float).reshape(-1)
        if x_current.size != self.nx:
            raise ValueError(
                f"x_current must have {self.nx} elements, got {x_current.size}. "
                f"Value={x_current}"
            )
        x_current = x_current.reshape(self.nx)

        pose_desired = np.asarray(pose_desired, dtype=float).reshape(-1)
        if pose_desired.size != 3:
            raise ValueError(
                f"pose_desired must have 3 elements [x,z,phi], got {pose_desired.size}. "
                f"Value={pose_desired}"
            )
        pose_desired = pose_desired.reshape(3,)
