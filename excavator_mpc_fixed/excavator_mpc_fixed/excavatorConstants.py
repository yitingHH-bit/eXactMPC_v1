...
This project contains *two* geometric models:

1) The original MATLAB-based excavator model (lenBA/lenAL/lenLM, etc.).
   It is still used by the actuator/cylinder mapping and inverse dynamics.

2) A **URDF-fitted planar kinematics model** used by:
   - forwardKinematics / inverseKinematics (end-effector pose)
   - MPC cost and GUI visualization

The URDF (model2_simple / robot.urdf) has joint-to-joint translations that are
*not* aligned with each link's +X axis (e.g., the lift->tilt vector has both x and z).
To reduce mismatch, we model each link as a fixed vector in its local frame:

    v1 = [0.20564,  0.420085]  (lift_boom -> tilt_boom joint, in lift_boom frame)
    v2 = [0.2496772, -0.0024304] (tilt_boom -> tool_body joint, in tilt_boom frame)

and a TCP offset vector from tool_body to the "tip" (chosen as the midpoint between claws):

    tool_body -> gripper_frame: [0.0363731, -0.0450618]
    gripper_frame -> claw_mid:  [(0.0261671-0.0238304)/2, -0.0385303]

The FK/IK implementation uses (L1, off1), (L2, off2) and (tcp_dx, tcp_dz).
"""

# --- URDF-fitted planar vectors (x,z) ---
URDF_V1 = np.array([0.20564, 0.420085], dtype=float)
URDF_V2 = np.array([0.2496772, -0.0024304], dtype=float)

L1 = float(np.linalg.norm(URDF_V1))
L2 = float(np.linalg.norm(URDF_V2))

# Fixed direction offsets of each link vector when joint angle = 0.
OFF1 = float(np.arctan2(URDF_V1[1], URDF_V1[0]))
OFF2 = float(np.arctan2(URDF_V2[1], URDF_V2[0]))

# TCP offset (tool_body frame) to a reasonable "gripper center".
_tool_to_gripper_frame = np.array([0.0363731, -0.0450618], dtype=float)
_gripper_frame_to_claw_mid = np.array([(0.0261671 - 0.0238304) / 2.0, -0.0385303], dtype=float)
TCP_VEC = _tool_to_gripper_frame + _gripper_frame_to_claw_mid
TCP_DX = float(TCP_VEC[0])
TCP_DZ = float(TCP_VEC[1])

# For quick reachability checks / plotting (magnitude only)
L3 = float(np.linalg.norm(TCP_VEC))
