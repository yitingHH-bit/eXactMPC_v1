...
# ============ 2D 模型里用到的 “几何长度” ============
LEN_BA = float(C.L1)   # |v1|  lift_boom -> tilt joint (URDF-fitted)
LEN_AL = float(C.L2)   # |v2|  tilt_boom -> scoop joint (URDF-fitted)
LEN_LM = float(C.L3)   # |tcp| tool_body -> chosen TCP (URDF-fitted)
...
class ExcavatorRealtimeGUI:
    ...
    def link_positions_3d(self, yaw, q):
        ...
        # URDF-fitted drawing (matches excavatorModel.forwardKinematicsPlanar):
        # link1 direction offset OFF1, link2 direction offset OFF2,
        # TCP offset (TCP_DX/TCP_DZ) rotated by tool orientation phi.
        p0 = np.array([0.0, 0.0, 0.0])
        p1 = p0 + np.array([
            LEN_BA * math.cos(alpha + float(C.OFF1)),
            0.0,
            LEN_BA * math.sin(alpha + float(C.OFF1)),
        ])
        p2 = p1 + np.array([
            LEN_AL * math.cos(alpha + beta + float(C.OFF2)),
            0.0,
            LEN_AL * math.sin(alpha + beta + float(C.OFF2)),
        ])

        # TCP point (chosen as gripper center)
        dx, dz = float(C.TCP_DX), float(C.TCP_DZ)
        p3 = p2 + np.array([
            dx * math.cos(phi) - dz * math.sin(phi),
            0.0,
            dx * math.sin(phi) + dz * math.cos(phi),
        ])
